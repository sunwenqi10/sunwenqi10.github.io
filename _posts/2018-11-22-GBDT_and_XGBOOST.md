...
2. for m=1 to M:
  + for k=1 to K:
    + compute $$g_{m-1,ki}$$ for each sample $$(x_i, y_i)$$
    + build up regression tree $$R_{mkj}$$(j=1 to J<sub>mk</sub> refer to the leaf nodes) from training samples $$(x_i, g_{m-1,ki})_{i=1 to N}$$
  +  
