...
2. for m=1 to M:
  ...
  + for k=1 to K: build up regression tree $$R_{mkj}$$(j=1 to J<sub>mk</sub> refer to the leaf nodes) from training samples $$(x_i, g_{m-1,ki})_{i=1 to N}$$
  + for k=1 to K: compute leaf weights $$w_{mkj}$$ for j=1 to J<sub>mk</sub>, $$w_{mkj}=argmin_{w}\sum_{{i}\in{R_{mkj}}}L(y_i, F_{m-1,1}(x_i), ..., F_{m-1,k}(x_i)+w, ...,  F_{m-1,K}(x_i))$$   
